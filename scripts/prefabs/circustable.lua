local assets =
{
    Asset("ANIM", "anim/circustable.zip"),
...
local function OnDecorGiven(inst, item, giver)
    ...
    inst:DoTaskInTime(0, function()
        if item and item:IsValid() and item.Follower then
            item.Follower:FollowSymbol(inst.GUID, nil, 0, -165, 0)  -- -200 Y offset
...
local function fn(Sim)
    ...
    inst.AnimState:SetBank("circustable")
    inst.AnimState:SetBuild("circustable")
	inst.AnimState:PlayAnimation("idle")
...
return Prefab( "circustable", fn, assets, prefabs),
       MakePlacer( "circustable_placer", "circustable", "circustable", "idle" ) 