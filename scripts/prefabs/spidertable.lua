local assets =
{
    Asset("ANIM", "anim/spidertable.zip"),
...
local function OnDecorGiven(inst, item, giver)
    ...
    inst:DoTaskInTime(0, function()
        if item and item:IsValid() and item.Follower then
            item.Follower:FollowSymbol(inst.GUID, nil, 0, -230, 0)  -- -200 Y offset
...
local function OnLoadPostPass(inst, newents, data)
    ...
    if inst.components.furnituredecortaker and inst.components.furnituredecortaker.decor_item then
...
local function fn(Sim)
    ...
    inst.AnimState:SetBank("spidertable")
    inst.AnimState:SetBuild("spidertable")
	inst.AnimState:PlayAnimation("idle")
...
return Prefab( "spidertable", fn, assets, prefabs),
       MakePlacer( "spidertable_placer", "spidertable", "spidertable", "idle" ) 